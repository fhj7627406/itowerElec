...
@SuppressWarnings("rawtypes") public class MapperMethod {
	...
	public MapperMethod(Class<?> declaringInterface, Method method, SqlSession sqlSession) {
		...
		// @MODIFIED
		isCustomed = willCallAnotherSql(method);
		if (!isCustomed) {
			setupFields();
	...
	public Object execute(Object[] args) {
		// @MODIFIED
		if (isCustomed) {
			setupFieldsByCustom(args);
			setupCommandType();
			validateStatement();

			// @MODIFIED
			// 第一个参数是sqlMapId,必须移出参数列表
			int tempLength = args.length - 1;
			Object[] tempArgs = new Object[tempLength];
			System.arraycopy(args, 1, tempArgs, 0, tempLength);
			args = tempArgs;
		}
	...
	private void setupMethodSignature() {
		...
		// @MODIFIED
		// 第一个参数是sqlId, String类型的，不能和参数列表一起处理
		if (isCustomed) {
			int argLength = argTypes.length - 1;
			if (argLength < 0) {
				throw new BindingException("Method to execute customed sql needs a sqlMapId: " + method.getName());
			}
			final Class<?>[] argTypesWithNoSqlId = new Class<?>[argLength];
			System.arraycopy(argTypes, 1, argTypesWithNoSqlId, 0, argLength);
			argTypes = argTypesWithNoSqlId;
		}
	...
	// -----------------------------customed field and methods

	// add this to test if this is a method to call some other sql statement
	private boolean isCustomed;
	private static final String CUSTOMED_TOKEN = "ByCustom";

	private boolean willCallAnotherSql(Method method) {
		return method.getName().indexOf(CUSTOMED_TOKEN) > 0;
	}

	private void setupFieldsByCustom(Object[] args) {
		if (args == null || !(args[0] instanceof String)) {
			throw new BindingException("Method to execute customed sql needs a sqlMapId: " + method.getName());
		}
		this.commandName = declaringInterface.getName() + "." + args[0];
	}
}